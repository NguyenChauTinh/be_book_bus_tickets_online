...
const clearNhanVienCache = async () => {
  try {
    const keys = await redisClient.keys("nhanvien_list:*");
    if (keys.length > 0) {
      await redisClient.del(keys);
      console.log("Đã xóa cache danh sách nhân viên");
    }
  } catch (error) {
    console.error("Lỗi khi xóa cache nhân viên:", error);
  }
};
...
export const layDanhSachNhanVien = async (req, res) => {
  try {
    ...
    const keyHoatDong = hoatDong !== undefined ? hoatDong : "all";
    const keyLoai = loaiNhanVien ? loaiNhanVien.toUpperCase() : "all";
    const redisKey = `nhanvien_list:${keyHoatDong}:${keyLoai}`;
    ...
    const danhSachNhanVien = await NhanVien.find(filter).sort({
      createdAt: -1,
    });

    if (danhSachNhanVien) {
      await redisClient.setEx(redisKey, 3600, JSON.stringify(danhSachNhanVien));
    }
...
export const taoNhanVien = async (req, res) => {
  try {
    const { maNhanVien, tenNhanVien, soDienThoai, namSinh, loaiNhanVien } =
      req.body;
...
export const capNhatNhanVien = async (req, res) => {
  try {
    const { id } = req.params;
    const { tenNhanVien, soDienThoai, namSinh, loaiNhanVien, email, diaChi } =
      req.body;
...
export const capNhatTrangThaiNhanVien = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: `Trạng thái nhân viên đã được cập nhật thành ${
        hoatDong ? "hoạt động" : "không hoạt động"
      }.`,
      data: nhanVien,
    });
  } catch (error) {
