    ...
    try {
        ...
        if (!Array.isArray(soDoGhe) || soDoGhe.length === 0) {
            return res.status(400).json({ 
                message: 'Sơ đồ ghế (soDoGhe) là bắt buộc và phải là một mảng không rỗng.' 
            });
        }
...
export const de_reactivateLoaiXe = async (req, res) => {
    try {
        ...
        if (typeof trangThai !== 'boolean') {
            return res.status(400).json({ message: 'Thuộc tính "trangThai" phải là boolean.' });
        }
...
export const getAllLoaiXe = async (req, res) => {
    try {
        ...
        const loaiXes = await LoaiXe.find(filter);
        res.status(200).json({ data: loaiXes });
    } catch (error) {
        res.status(500).json({ message: 'Lỗi khi lấy danh sách loại xe', error: error.message });
    }
};
